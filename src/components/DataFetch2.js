...
// (coordinates) temp[0] = {lat: xxx , lon: xxx} 
// (time) temp[1] = {time: xxx}
// (temp) temp[2] = {temperature: xxx , weatherID: xxx , descrip: xxx}
    ...
    return temp.length==0 ? <span>Loading</span> :(
        <div>
            <form>
            <input type={"text"} placeholder='Longitude' onChange={handleLong}></input>
            <input type={"text"} placeholder='Latitude' onChange={handleLat}></input>
            <h1>Long is {long}</h1>
            <h1>Lat is {lat}</h1>
            </form>
                <h2>{temp[1].time}</h2>
